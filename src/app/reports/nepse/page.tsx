...
export const metadata: Metadata = {
  title: 'NEPSE Market Reports — The.Nerds | Nepal Stock Exchange Analysis',
  description:
    'In-depth NEPSE market reports: sector analysis, index forecasts, IPO assessments, and weekly market intelligence for Nepal Stock Exchange investors.',
  alternates: { canonical: '/reports/nepse' },
  openGraph: {
    title: 'NEPSE Market Reports — The.Nerds',
    description: 'In-depth Nepal Stock Exchange analysis and research reports.',
...
const nepseSchema = {
  ...
  '@type': 'CollectionPage',
  name: 'NEPSE Market Reports — The.Nerds',
  description: 'In-depth Nepal Stock Exchange analysis and research reports.',
  url: `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:4028'}/reports/nepse`,
};

export default async function NepseReportsPage() {
  const reports = await fetchPublishedReports('nepse');

  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(nepseSchema) }}
      />
      <Header />
        ...
        <section className="relative overflow-hidden border-b border-border/40">
          ...
          <div className="max-w-7xl mx-auto px-6 py-16 md:py-20">
            <nav className="flex items-center gap-2 text-xs text-muted-foreground mb-8" aria-label="Breadcrumb">
              ...
              <span>/</span>
              <span className="text-foreground">NEPSE Market</span>
            </nav>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                ...
                <p className="text-muted-foreground max-w-2xl leading-relaxed">
                  Institutional-grade research on Nepal Stock Exchange, directly synced from your dashboard.
                </p>
              </div>
        ...
        <section className="max-w-7xl mx-auto px-6 py-12 md:py-16">
          ...
          {reports.length === 0 ? (
            <div className="rounded-xl border border-border bg-card px-4 py-8 text-center text-sm text-muted-foreground">
              No NEPSE reports are published yet.
            </div>
          ) : (
                  ...
                  <div className="flex flex-col md:flex-row md:items-start gap-5">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2.5 mb-3">
                        <span className="text-xs font-bold uppercase tracking-wider text-primary">NEPSE</span>
