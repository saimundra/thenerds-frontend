...
export const metadata: Metadata = {
  title: 'SMC Analysis Reports — The.Nerds | Smart Money Concepts Research',
  description:
    'Smart Money Concepts research reports: institutional order flow, liquidity sweeps, order blocks, fair value gaps, and market structure analysis for professional traders.',
  alternates: { canonical: '/reports/smc' },
  openGraph: {
    title: 'SMC Analysis Reports — The.Nerds',
    description: 'Institutional order flow and Smart Money Concepts research reports.',
...
const smcSchema = {
  ...
  '@type': 'CollectionPage',
  name: 'SMC Analysis Reports — The.Nerds',
  description: 'Institutional order flow and Smart Money Concepts research reports.',
  url: `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:4028'}/reports/smc`,
};

export default async function SmcReportsPage() {
  const reports = await fetchPublishedReports('smc');

  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(smcSchema) }}
      />
      <Header />
        ...
        <section className="relative overflow-hidden border-b border-border/40">
          ...
          <div className="max-w-7xl mx-auto px-6 py-16 md:py-20">
            <nav className="flex items-center gap-2 text-xs text-muted-foreground mb-8" aria-label="Breadcrumb">
              ...
              <span>/</span>
              <span className="text-foreground">SMC Analysis</span>
            </nav>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                ...
                <h1 className="text-3xl md:text-5xl font-bold text-foreground tracking-tight mb-4">
                  SMC Analysis Reports
                </h1>
                <p className="text-muted-foreground max-w-2xl leading-relaxed">
                  Institutional order flow reports published from your dashboard.
                </p>
              </div>
        ...
        <section className="border-b border-border/40 bg-card/20">
          <div className="max-w-7xl mx-auto px-6 py-5">
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
              <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold shrink-0">Key Concepts:</span>
              {['Order Block (OB)', 'Fair Value Gap (FVG)', 'Liquidity Sweep', 'CHoCH / BOS', 'Premium & Discount', 'Inducement'].map((c) => (
                <span key={c} className="text-xs text-primary/80 bg-primary/10 border border-primary/20 px-2.5 py-1 rounded-md">
                  {c}
                </span>
              ))}
            </div>
        ...
        <section className="max-w-7xl mx-auto px-6 py-12 md:py-16">
          ...
          {reports.length === 0 ? (
            <div className="rounded-xl border border-border bg-card px-4 py-8 text-center text-sm text-muted-foreground">
              No Smart Money Concepts reports are published yet.
            </div>
          ) : (
                  ...
                  <div className="flex flex-col md:flex-row md:items-start gap-5">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2.5 mb-3">
                        <span className="text-xs font-bold uppercase tracking-wider text-primary">SMC</span>
